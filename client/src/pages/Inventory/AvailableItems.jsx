...
export default function AvailableItems() {
    ...
    return (
      ...
      >
        <div className="relative bg-surface-container-low dark:bg-[#1a1a1a] rounded-lg sm:rounded-xl overflow-hidden border border-transparent hover:border-primary/20 dark:border-gray-700 dark:hover:border-blue-500/50 transition-all shadow-sm hover:shadow-md dark:hover:shadow-lg dark:hover:shadow-black/40 h-full min-h-[170px] xs:min-h-[185px] sm:min-h-[220px] md:min-h-[260px] flex flex-col">
          ...
          <div className="relative h-20 xs:h-22 sm:h-28 md:h-48 bg-surface-container-high dark:bg-[#222] overflow-hidden">
            {imageUrl ? (
              <img
                src={imageUrl}
                alt={item.name}
          ...
          <div className="p-2 xs:p-2.5 sm:p-3 md:p-4 flex-grow flex flex-col justify-between">
            <div>
              <p className="text-[8px] xs:text-[9px] sm:text-xs text-on-surface-variant dark:text-gray-400 uppercase tracking-wide mb-0.5 sm:mb-1 leading-3">
                {divisionName ? `${divisionName} • ${categoryLabel}` : categoryLabel}
              </p>
              <h3 className="text-[10px] xs:text-[11px] sm:text-sm font-bold text-on-surface dark:text-white line-clamp-2 leading-3.5 xs:leading-4 sm:leading-5 mb-1 sm:mb-2">
                {item.name}
              </h3>
              {divisionName && (
                <span className="inline-flex items-center rounded-full border border-primary/20 bg-primary/10 px-2 py-0.5 text-[7px] xs:text-[8px] sm:text-[10px] font-medium text-primary dark:border-blue-400/20 dark:bg-blue-500/10 dark:text-blue-300 mb-1">
                  {divisionName}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center justify-between mt-1">
              <p className={`text-[8px] xs:text-[9px] sm:text-xs font-medium ${isAvailable ? 'text-primary dark:text-blue-400' : 'text-on-surface-variant dark:text-gray-400'}`}>
  ...
  const renderItemCards = (itemsList) => (
    <div className="grid grid-cols-2 gap-1.5 xs:gap-2 sm:gap-3 md:grid-cols-3 lg:grid-cols-5 md:gap-4 pb-4 xs:pb-5 sm:pb-6 md:pb-8">
      {itemsList.map((item) => {
