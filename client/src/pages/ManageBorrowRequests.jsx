...
export default function ManageBorrowRequests() {
  ...
  const [actionLoading, setActionLoading] = useState({}); // Track loading for each request
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white shadow rounded mt-6">
      <h1 className="text-3xl font-bold text-blue-600 mb-6">Manage Borrow Requests</h1>
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-600">
                  Requested: {new Date(req.request_date).toLocaleString()}
                </p>
              ...
              {req.status === "pending" && (
                <div className="mt-3 md:mt-0 flex flex-col items-start gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setDueDates((prev) => ({ ...prev, [req.id]: e.target.value }))
                    }
                  />
                  <button
                    ...
                    className={`bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 w-full ${
                      actionLoading[req.id] ? "opacity-50 cursor-not-allowed" : ""
                    }`}
                  >
